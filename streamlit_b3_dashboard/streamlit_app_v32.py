            ...
            try:
                df = yf.download(ativo, start=data_inicio, end=data_fim)
                    ...
                    df = df[[col for col in colunas_esperadas if col in df.columns]]
                        if df.empty:
                    st.warning(f"⚠️ Dados ausentes ou incompletos para o ativo {ativo}.")
                    continue
                    ...
                    cerebro.addstrategy(estrategia_selecionada)
                ...
                resultado = cerebro.run()[0]
                equity = resultado.analyzers.equity.equity
                ax.plot(equity, label=ativo)
    
                resultados.append({
                    'Ação': ativo,
                    'Retorno Total (R$)': round(equity[-1] - 10000, 2),
                    'Sharpe': round(resultado.analyzers.sharpe.get_analysis().get('sharperatio', 0), 2),
                    'Drawdown (%)': round(resultado.analyzers.drawdown.get_analysis()['max']['drawdown'], 2)
                })
