...
if executar and ativos:
    ...
    for ativo in ativos:
        ...
        if isinstance(df.columns, pd.Index) and all(isinstance(col, str) for col in df.columns):
            df.columns = [col.strip() for col in df.columns]
        else:
            st.error(f"Erro ao processar colunas do ativo {ativo}: formato inesperado.")
            continue
